...
x_real_score = d_model([x_in, cond_in])
...
x_fake_score = d_model([g_model([z_in, cond_in]), cond_in])

g_train_model = Model([z_in,cond_in], x_fake_score)
g_train_model.add_loss(K.mean(- K.log(x_fake_score + 1e-9)))
g_train_model.compile(optimizer=Adam(2e-4, 0.5))

g_train_model.load_weights("/rap_blues/MNIST/cgan_images2/samples/g_train_model.weights")
...
for i in range(total_iter):
    ...
    if i % iters_per_sample == 0:
        sample('/rap_blues/MNIST/cgan_test/samples/test_%s.png' % i)
        g_train_model.save_weights('/rap_blues/MNIST/cgan_test/samples/g_train_model.weights')