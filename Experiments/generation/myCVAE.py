...
def sample(path):
    ...
    for i in range(n):
        for j in range(n):
            ...
            x_recon = decoder.predict([yy, np.random.randn(1, *K.int_shape(x)[1:])])
            digit = x_recon[0]
...
vae.fit([x_train, y_train], batch_size=32, shuffle=False, callbacks=[evaluator], epochs=3000)
