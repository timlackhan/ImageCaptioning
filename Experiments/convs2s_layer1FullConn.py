...
image_conv3 = Conv2D(32, (3,3), activation='relu', padding='same')(image_conv2)
image_conv4 = Conv2D(32, (3,3), activation='relu', padding='same', strides=2)(image_conv3)
image_conv5 = Conv2D(64, (3,3), activation='relu', padding='same')(image_conv4)
image_conv6 = Conv2D(64, (3,3), activation='relu', padding='same', strides=2)(image_conv5)
image_conv7 = Conv2D(128, (3,3), activation='relu', padding='same')(image_conv6)
image_conv8 = Flatten()(image_conv7)
image_conv9 = Dense(1024, activation='relu')(image_conv8)
image_conv10 = Dropout(0.3)(image_conv9)
image_conv11 = Dense(1024, activation='relu')(image_conv10)
image_conv12 = Dropout(0.3)(image_conv11)
image_conv13 = Reshape((1024, 1))(image_conv12)
image_encoder = Dense(21, activation='relu')(image_conv13)
...
my_model.layers[1].set_weights(model.layers[4].layers[0].get_weights())
my_model.layers[2].set_weights(model.layers[4].layers[1].get_weights())
my_model.layers[3].set_weights(model.layers[4].layers[2].get_weights())
my_model.layers[4].set_weights(model.layers[4].layers[3].get_weights())
my_model.layers[5].set_weights(model.layers[4].layers[4].get_weights())
my_model.layers[6].set_weights(model.layers[4].layers[5].get_weights())
my_model.layers[7].set_weights(model.layers[4].layers[6].get_weights())
my_model.layers[10].set_weights(model.layers[4].layers[8].get_weights())
my_model.layers[15].set_weights(model.layers[4].layers[10].get_weights())

#train
optimizer = RMSprop(lr=0.0001, clipvalue=1.0)
my_model.compile(loss='categorical_crossentropy', optimizer=optimizer)
filepath="/rap_blues/lunwen/convs2s/full/0/org-weights-epoch-{epoch:02d}--val_loss-{val_loss:.4f}--loss-{loss:.4f}.hdf5"
...
my_model.fit([image_data, X, padding_up], y, batch_size=64, shuffle=False, validation_split=0.1, callbacks=callbacks_list, verbose=1, epochs=100)
