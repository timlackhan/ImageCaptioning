from keras.layers import add, Embedding, TimeDistributed, RepeatVector, LSTM, GRU, concatenate , Input, Reshape, Dense,Activation,Dropout,Flatten,Conv2D,MaxPooling2D
...
my_model.layers[3].set_weights(model.layers[3].get_weights())
my_model.layers[4].set_weights(model.layers[4].get_weights())
my_model.layers[5].set_weights(model.layers[5].get_weights())
my_model.layers[11].set_weights(model.layers[7].get_weights())
my_model.layers[12].set_weights(model.layers[8].get_weights())

optimizer = RMSprop(lr=0.0001, clipvalue=1.0)
my_model.compile(loss='categorical_crossentropy', optimizer=optimizer)
filepath="/rap_blues/lunwen/baseline_attention/0/org-weights-epoch-{epoch:04d}--val_loss-{val_loss:.4f}--loss-{loss:.4f}.hdf5"
...
my_model.fit([image_data, X], y, batch_size=64, shuffle=False, validation_split=0.1, callbacks=callbacks_list, verbose=1, epochs=100)
