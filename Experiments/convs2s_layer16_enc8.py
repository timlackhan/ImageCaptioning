...
image_conv1 = Conv2D(16, (3,3), padding='valid', activation='relu', strides=2)(image_input)
image_conv2 = Conv2D(16, (3,3), activation='relu', padding='same')(image_conv1)
image_conv3 = Conv2D(32, (3,3), activation='relu', padding='same', strides=2)(image_conv2)
image_conv4 = Conv2D(32, (3,3), activation='relu', padding='same')(image_conv3)
image_conv5 = Conv2D(64, (3,3), activation='relu', padding='same', strides=2)(image_conv4)
image_conv6 = Conv2D(64, (3,3), activation='relu', padding='same')(image_conv5)
image_conv7 = Conv2D(128, (3,3), activation='relu', padding='same', strides=2)(image_conv6)
image_conv8 = Conv2D(21, (3,3), activation='relu', padding='same')(image_conv7)
image_encoder = Reshape((256,21))(image_conv8)
...
decoder_softmax = Dense(21, activation='softmax')(decoder_17i) 
#model
my_model = Model(inputs=[image_input, language_input, padding_input], outputs=decoder_softmax)
my_model.load_weights("/rap_blues/lunwen/convs2s/layer16/org-weights-epoch-485--val_loss-0.6210--loss-0.1125.hdf5")
...
my_model.layers[5].set_weights(model.layers[4].layers[3].get_weights())
my_model.layers[8].set_weights(model.layers[4].layers[4].get_weights())
my_model.layers[10].set_weights(model.layers[4].layers[5].get_weights())
my_model.layers[13].set_weights(model.layers[4].layers[6].get_weights())

#train
optimizer = Adam(lr=0.0001, clipvalue=1.0)
my_model.compile(loss='categorical_crossentropy', optimizer=optimizer)
filepath="/rap_blues/lunwen/convs2s/layer16_enc16/org-weights-epoch-{epoch:02d}--val_loss-{val_loss:.4f}--loss-{loss:.4f}.hdf5"
